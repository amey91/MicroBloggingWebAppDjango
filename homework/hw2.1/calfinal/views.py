...
def hello(request):
	...
	if '1' in request.POST:
		name_to_greet=request.POST['num1']
		if name_to_greet == '0':
			name_to_greet='1'
		else: name_to_greet = name_to_greet + str(1)
		j=int(name_to_greet)
		
		'''name_to_greet = request.POST['1']
		...
		if 'num1' in request.POST:
			if request.POST['num1']=="": 
				request.POST['num1']=0
			name_to_greet= int(request.POST['num1']) + request.POST['1']'''
		...
		name_to_greet = appendnum(request.POST['num1'],7)  # got tired of copy pasting and then took the long way around 
		
	elif '8' in request.POST:
		name_to_greet = appendnum(request.POST['num1'],8)
		
		
	elif '9' in request.POST:
		name_to_greet = appendnum(request.POST['num1'],9)
	elif '0' in request.POST:
		name_to_greet = appendnum(request.POST['num1'],0)
		
	elif '+' in request.POST:
		c=cal.objects.get(id=1)
		c.ans=request.POST['num1']
		c.op="+"
		c.save()		
		name_to_greet = 0
	# Makes the data available to the view as 'person_name', then renders the view
		...
		if c.op=='':
			name_to_greet=request.POST['num1']
		if c.op== "+":
			name_to_greet=int(request.POST['num1'])+int(c.ans)
			c.ans=0
			c.op=""
			c.save()
		elif c.op=="/":
			name_to_greet=int(c.ans)/int(request.POST['num1'])
			c.ans=0
			c.op=""
			c.save()
		elif c.op=="-":
